...
export class Grid{
  ...
  collide(mat,x,y){ for(let j=0;j<4;j++) for(let i=0;i<4;i++) if(mat[j][i]){ if(!this.inside(x+i,y+j) || this.cells[y+j][x+i]) return true; } return false; }
  merge(mat,x,y,color){ for(let j=0;j<4;j++) for(let i=0;i<4;i++) if(mat[j][i]) this.cells[y+j][x+i]=color||1; }
